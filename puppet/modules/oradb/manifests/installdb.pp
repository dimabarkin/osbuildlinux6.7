...
)
{
  ...
  if ( $continue ) {
    ...
    if ( $zip_extract ) {
      ...
      info "/usr/bin/unzip -o ${source}/${file1} -d ${download_dir}/${file}"
      exec { "extract ${download_dir}/${file1}":
        command   => "/usr/bin/unzip -o ${source}/${file1} -d ${download_dir}/${file}",
        timeout   => 0,
        logoutput => true,
        path      => $execPath,
        user      => $user,
      ...
      exec { "extract ${download_dir}/${file2}":
        command   => "/usr/bin/unzip -o ${source}/${file2} -d ${download_dir}/${file}",
        timeout   => 0,
        logoutput => true,
        path      => $execPath,
        user      => $user,
